...
class ModelTests(TestCase):
    ...
    def test_ingredient_str(self):
        ...
        ingredient = models.Ingredient.objects.create(user=sample_user(), name="Cheese")
        self.assertEqual(str(ingredient), ingredient.name)

    def test_recipe_str(self):
        """Test recipe str representation"""
        recipe = models.Recipe.objects.create(
            user=sample_user(), title="Combinaton Pasta", time_minutes=5, price=5.00
        )
        self.assertEqual(str(recipe), recipe.title)

    @patch("uuid.uuid4")
    def test_recipe_filename_uuid(self, mock_uuid):
        """Test image is been saved in correct dir"""
        uuid = "test-uuid"
